...
class IsSameDayIsNotSameDayTest extends TestCase
{
     ...
     * @dataProvider provide
     */
    public function testIsSameDayWithValueHolder($param, $psiValue, $expectedResult)
    {
        $subject = new IsSameDay(
            new GenericHolder($param ? new LocalDate($param, 'Etc/UTC') : null)
        );
        $result  = $subject->__invoke($psiValue ? new LocalDate($psiValue, 'Etc/UTC') : null);
     ...
     * @dataProvider provide
     */
    public function testIsNotSameDayWithValueHolder($param, $psiValue, $expectedResult)
    {
        $expectedResult = ! $expectedResult;

        $subject = new IsNotSameDay(
            new GenericHolder($param ? new LocalDate($param, 'Etc/UTC') : null)
        );
        $result  = $subject->__invoke($psiValue ? new LocalDate($psiValue, 'Etc/UTC') : null);
